...
def main(resources_dir):
    df_logs = pd.read_csv(f"{resources_dir}/linux/log-templates/Linux.log_structured.csv")
    ...
    plt.savefig(f"{resources_dir}/images/severity_counts.png", dpi=300, bbox_inches="tight")

    event_matrix.to_csv(f"{resources_dir}/linux/log-structured/Linux.log_sequences.csv", index=False)
